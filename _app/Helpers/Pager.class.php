...
class Pager {
     ...
     * Informe o nome da tabela e condições caso exista. O resto é feito pelo método. Execute um <b>echo getPaginator();</b>
     * para exibir a paginação na view.
     * @param STRING $FullRead = Read
     * @param STRING $ParseString = Prepared Statements
     */
    public function ExeFullPaginator($FullRead, $ParseString = null) {
        $this->FullRead = (string) $FullRead;
        $this->Places = (string) $ParseString;
        $this->getSyntax();
    }
    ...
    private function getSyntax() {
        $read = new Read;
        if(!empty($this->FullRead)):
            $read->FullRead($this->FullRead, $this->Places);
        else:
        ...
        if ($this->Rows > $this->Limit):
            ...
            $this->Paginator = "<div class=\"row\">";
            $this->Paginator .= "<div class=\"col-md-12 text-center\">";
            $this->Paginator .= "<nav>";
            $this->Paginator .= "<ul class=\"pagination\">";
            $this->Paginator .= "<li><a title=\"{$this->First}\" href=\"{$this->Link}1\" aria-label=\"Primeiro\">{$this->First}</a></li>";
            ...
            $this->Paginator .= "<li><a title=\"{$this->Last}\" href=\"{$this->Link}{$Paginas}\" aria-label=\"Último\">{$this->Last}</a></li>";
            $this->Paginator .= "</ul>";
            $this->Paginator .= "</nav>";
            $this->Paginator .= "</div>";
            $this->Paginator .= "</div>";
